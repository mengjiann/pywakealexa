...
class AlexaDevice:
    ...
    def set_speech_instance(self, trigger_word):
        self.speech_instance = alexa_audio.Speech(trigger_word)
    ...
    def user_initiate_audio(self):
        ...
        raw_audio = self.speech_instance.get_audio()
    ...
    def process_directive_speaker(self, content, attachment):
        ...
        if name in ['SetVolume', 'AdjustVolume']:
            adjust_volume = payload['volume']/2
    ...
    def process_directive_audio_player(self, content, attachment):
        ...
        if name == 'Play':
            ...
            if attachment is None:
                ...
                self.player_instance.queued_play(audio_file, streamId=audio_id, offset=offset, report=report)
    ...
    def process_directive_speech_synthesizer(self, content, attachment):
        ...
        name = header['name']
        if 'playBehavior' in payload:
            if payload['playBehavior'] in ['REPLACE_ALL']:
                force = True
        if name == 'Speak':
            token = payload['token']
            if attachment:
                ...
                self.alexa.get_and_process_response(stream_id)
                self.player_instance.blocking_play(audio_file, streamId=token)
    ...
    def process_directive_speech_recognizer(self, content, attachment):
        ...
        if name == 'ExpectSpeech':
            ...
            raw_audio = self.player_instance.get_audio(timeout)
            if raw_audio is None:
